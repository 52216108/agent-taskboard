...
- 六列流转：已收集 → 待规划 → 待开发 → 进行中 → 待验收 → 已完成，每列一个状态色，列头可直接建卡
  （「已完成」列没有建卡入口——置为已完成只能由人从「待验收」验收）
- 优先级 / 类型（需求·缺陷·优化）/ 认领人 / 截止日期 / 图片附件
- 子任务：每个任务可挂一份轻量检查清单（卡片显示进度、详情弹窗里勾选、`board here` 缩进列出）
- 验收打回：待验收打回待开发时带原因，原因会回灌进 `board here` 输出给 agent 看；打回时可附截图，打回后仍可修订打回内容
- 跨项目全局任务视图
