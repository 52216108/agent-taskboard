...
**要分清哪些是强制的、哪些不是**：API 会校验状态值合法性、限定只有「待验收」的任务能被打回；
但*「agent 只领待开发、干完置待验收而不是直接置已完成」*是写在 `AGENTS.md` 和 Claude Code skill 里的
**工作流约定**——agent 若不遵守，API 并不会拒绝。验收本意是人的动作，但目前这是规范，不是锁。
